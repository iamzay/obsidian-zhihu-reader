...
interface CompatibleSettingDefinition {
  ...
  readonly desc?: string | DocumentFragment;
  readonly visible?: boolean | (() => boolean);
  readonly render?: (setting: Setting) => void | (() => void);
}

interface CompatibleSettingGroup {
  readonly type: "group";
  readonly heading?: string;
  readonly cls?: string;
  readonly items: readonly CompatibleSettingDefinition[];
}
...
export class ZhihuAnswersSettingTab extends PluginSettingTab {
  ...
  /**
   * Obsidian 1.13+ consumes these definitions for rendering and settings
   * search. Older versions ignore this method and continue to call display().
   */
  ...
  override display(): void {
    ...
    for (const group of this.getSettingDefinitions()) {
      ...
      const groupContainer = group.cls === undefined
        ? containerEl
        : containerEl.createDiv({ cls: group.cls });
      for (const definition of group.items) {
        if (!isVisible(definition.visible)) {
          continue;
        }
        const setting = new Setting(groupContainer).setName(definition.name);
  ...
  private authSettingDefinitions(): CompatibleSettingDefinition[] {
